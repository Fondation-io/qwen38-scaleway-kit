  ...
  const { filteredIndices, groups } = useMemo(() => {
    ...
    const filteredIndices = threadIds
      ...
      .filter(
        ({ id }) =>
          !query ||
          (itemsById.get(id)?.title || "Nouvelle conversation")
            .toLowerCase()
            .includes(query),
      )
      .map(({ index }) => index);
