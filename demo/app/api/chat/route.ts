...
- SECAUDIT.DAILY_BASELINE(user_profile, day, stream, n_events, mean_events, std_events) : agrégat quotidien par profil et flux (stream ∈ signon, transfer_session, mail, object_transfer), day ISO YYYY-MM-DD, avec moyenne et écart-type PROPRES au profil. ATTENTION : la vue contient les jours actifs seulement ; les jours à zéro sont absents. Pour un flux rare, compte les jours actifs sur toute la période avant de parler de normalité ; un seul jour actif avec écart-type nul reste un signal rare.
...
- Dans HONEYPOT, job_name seul ne prouve jamais le service. Conserve le nom technique ; ne le traduis que si une autre colonne du même événement documente le service. remote_port est le port source du client, souvent éphémère ; seul local_port peut documenter le service visé, et une valeur vide reste inconnue.
